...
# 配置路径
working_directory = "/public/home/hpc221253/specfem3d/specfem3d-master/EXAMPLES/30-500menyuan4"
...
output_hdf5_path = "/public/home/hpc221253/pytorch_gpu/specfem3d-py4/data_3d"
...
def submit_specfem_job():
    ...
    try:
        ...
        if job_id_match:
            ...
            time.sleep(10)  # 提交作业后等待10秒
            return job_id
        else:
            logging.error("Failed to extract job ID from submission output.")
            return None
...
def is_job_running(job_id):
    command = ["squeue", "--job", job_id]
    try:
        result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        return job_id in result.stdout
    except Exception as e:
        logging.error(f"An error occurred while checking job status: {e}")
        return False

# 获取作业状态
def get_job_status(job_id):
    command = ["sacct", "-j", job_id, "--format=State"]
    try:
        result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        status_line = result.stdout.splitlines()[2]  # 第三行是状态信息
        status = status_line.strip().split()[-1]
        return status
    except Exception as e:
        logging.error(f"An error occurred while retrieving job status: {e}")
        return None

# 检查SPECfem3D输出结果是否存在并完成
# 检查SPECfem3D输出结果是否存在并完成
def is_specfem_output_complete(expected_x, expected_y, expected_z, job_id):
    try:
        ...
        all_stations_exist = all(
            all(os.path.exists(os.path.join(output_files_dir, f"HD.CZ{station}.{component}.sema"))
                for component in ['CXX', 'CXY', 'CXZ'])
            for station in range(1, 4097)
        )

        # 如果任何一个文件不存在，返回 False
        if not cmtsolution_exists or not all_stations_exist:
        ...
        logging.info(f"All station files exist: {all_stations_exist}")
        logging.info(f"Parameters match: {parameters_match}")
        
        if not parameters_match:
            job_status = get_job_status(job_id)
            logging.info(f"Job status: {job_status}")
            if job_status not in ['PD', 'R', 'PENDING', 'RUNNING']:  # 如果作业状态不是排队或运行中
                logging.info("Job is not running, resubmitting...")
                return False  # 返回False以触发重新提交

        return cmtsolution_exists and all_stations_exist and parameters_match
...
def merge_hdf5_files(batch_index, num_simulations):
    merged_file_path = os.path.join(output_hdf5_path, f'displacement_data{batch_index+2}.h5')
        ...
        for batch in range(num_batches):
            for simulation in range(num_simulations):
                ...
                while job_id is None:
                    ...
                    if job_id is None:
                        logging.info("Job submission failed, retrying in 5 seconds...")
                        time.sleep(10)
                    else:
                        logging.info("Job successfully submitted.")
                        time.sleep(10)  # 提交作业后等待10秒

                # 检查作业状态，确保作业正确提交
                while not is_job_running(job_id):
                    logging.error("Job submission failed after initial confirmation, retrying submission.")
                    job_id = submit_specfem_job()
                    if job_id is None:
                        logging.info("Job submission failed, retrying in 5 seconds...")
                        time.sleep(10)
                ...
                # 等待作业完成，每100秒检查一次输出文件
                while True:
                    if is_specfem_output_complete(new_source_x1, new_source_y1, new_source_z1, job_id):
                        # 再次检查是否有空文件
                    ...
                    time.sleep(300)  # 以秒为单位等待

                displacement_data = process_displacement_data(output_files_dir)

                if displacement_data is None:
                    logging.error("Displacement data processing failed, stopping further execution.")
                    break
