...
class pointwise_op_3D(nn.Module):
    ...
    def forward(self,x, dim1 = None, dim2 = None, dim3 = None):
        ...
        ft = torch.fft.rfftn(x_out.float(), dim=[-3,-2,-1])
...
class Uno3D_T10(nn.Module):
    ...
    def get_grid(self, shape, device):   ###包含5个特征数量
        ...
        expected_shape = (1, size_x, size_y, size_z, 5)
        if self._coordinate_grid.shape != expected_shape or self._coordinate_grid.device != device:
            # Preserve the original NumPy-to-float32 coordinate values exactly;
            # construction and transfer now happen only once per grid shape.
            gridx = torch.tensor(np.linspace(0, 2 * np.pi, size_x), dtype=torch.float32)
            gridy = torch.tensor(np.linspace(0, 2 * np.pi, size_y), dtype=torch.float32)
            ...
            self._coordinate_grid = torch.cat(
                (
                    torch.sin(gridx).expand(1, size_x, size_y, size_z, 1),
                    torch.sin(gridy).expand(1, size_x, size_y, size_z, 1),
                    torch.cos(gridx).expand(1, size_x, size_y, size_z, 1),
                    torch.cos(gridy).expand(1, size_x, size_y, size_z, 1),
                    gridz.expand(1, size_x, size_y, size_z, 1),
                ),
                dim=-1,
            ).to(device)
        return self._coordinate_grid.expand(batchsize, -1, -1, -1, -1)
